...
def twilio_status_callback(request):
    """
    Callback for call status updates with intelligent scheduling
    🚀 Enhanced with automatic next-call scheduling based on customer response
    """
    call_sid = request.POST.get('CallSid', 'Unknown')
    ...
    try:
        ...
        # If call is completed, trigger intelligent scheduling
        if call_status.lower() == 'completed':
            ...
            # 🎙️ ISSUE #2 FIX: Capture Recording URL if available
            recording_url = request.POST.get('RecordingUrl')
            if recording_url:
                call.recording_url = recording_url
                logger.info(f"✅ Recording captured: {recording_url[:50]}...")
            
            # 🎙️ ISSUE #2 FIX: Capture HumeAI Session ID from cache if available
            from django.core.cache import cache
            hume_session = cache.get(f'hume_session_{call_sid}')
            if hume_session:
                call.hume_session_id = hume_session
                logger.info(f"✅ HumeAI session captured: {hume_session[:30]}...")
            
        call.save()
        logger.info(f"✅ Updated call {call_sid} status to {call_status}")
        
        # 🧠 INTELLIGENT AUTO-SCHEDULING: Analyze completed call and schedule next one
        if call_status.lower() == 'completed':
            from .intelligent_hume_scheduler import hume_twilio_scheduler
            
            try:
                # Trigger intelligent scheduling based on call outcome
                scheduling_result = hume_twilio_scheduler.analyze_hume_call_and_schedule(call)
                
                if scheduling_result['success']:
                    logger.info(f"✅ Auto-scheduled next call for {call.to_number}: {scheduling_result['analyzed_outcome']}")
                else:
                    logger.error(f"❌ Failed to auto-schedule: {scheduling_result.get('error')}")
                    
            except Exception as e:
                logger.error(f"❌ Intelligent scheduling error: {str(e)}")
