...
from .sales_script_generator import generate_sales_script
from .knowledge_to_script import generate_script_from_knowledge_file
import logging
...
class AgentCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # 🌐 AUTO-GENERATE SALES SCRIPT FROM WEBSITE URL
        website_url = validated_data.get('website_url')
        custom_script = validated_data.get('sales_script_text', '').strip()
        ...
        if website_url:
            ...
            try:
                ...
                if website_data.get('success'):
                    logger.info(f"✅ Successfully scraped website: {website_data.get('company_name')}")
                    
                    # Generate sales script from website
                    agent_name = validated_data.get('name', 'AI Agent')
                    agent_tone = validated_data.get('voice_tone', 'professional')
                    ...
                    # 🔥 CONCAT LOGIC: If user provided custom script, combine both
                    if custom_script:
                        # Custom script first (user's intro), then website script
                        combined_script = f"""{custom_script}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

{website_script}"""
                        validated_data['sales_script_text'] = combined_script
                        logger.info(f"✅ Combined custom + website script: {len(combined_script)} characters")
                    else:
                        # Only website script
                        validated_data['sales_script_text'] = website_script
                        logger.info(f"✅ Website script generated: {len(website_script)} characters")
                    
                    # Store website data in business_info for future reference
                    business_info = validated_data.get('business_info', {})
                    business_info['website_data'] = {
                        'company_name': website_data.get('company_name'),
                        'description': website_data.get('description'),
                        'products_services': website_data.get('products_services', [])[:5],
                        'key_features': website_data.get('key_features', [])[:5],
                        'contact_info': website_data.get('contact_info', {}),
                        'scraped_at': timezone.now().isoformat()
                    }
                    validated_data['business_info'] = business_info
                    
                else:
                    logger.warning(f"⚠️ Failed to scrape website: {website_data.get('error')}")
                    # If scraping failed but custom script exists, use it
                    if custom_script:
                        validated_data['sales_script_text'] = custom_script
                    
            except Exception as e:
                logger.error(f"❌ Error processing website URL: {e}")
                # If error but custom script exists, use it
                if custom_script:
                    validated_data['sales_script_text'] = custom_script
        ...
        if knowledge_files_upload:
            ...
            # 📚 AUTO-GENERATE SCRIPT FROM KNOWLEDGE FILES (if no script yet)
            if not agent.sales_script_text and knowledge_files_upload:
                logger.info(f"📚 No script found, attempting to generate from knowledge files")
                try:
                    # Try to generate script from first knowledge file
                    first_file = knowledge_files_upload[0]
                    
                    # Save temporarily to read
                    temp_path = f'/tmp/{first_file.name}'
                    with open(temp_path, 'wb+') as temp_file:
                        for chunk in first_file.chunks():
                            temp_file.write(chunk)
                    
                    # Generate script from file
                    knowledge_script = generate_script_from_knowledge_file(
                        file_path=temp_path,
                        agent_name=agent.name,
                        agent_tone=agent.voice_tone or 'professional'
                    )
                    
                    if knowledge_script:
                        agent.sales_script_text = knowledge_script
                        agent.save(update_fields=['sales_script_text'])
                        logger.info(f"✅ Sales script generated from knowledge file: {len(knowledge_script)} chars")
    ...
    def update(self, instance, validated_data):
        ...
        # 🌐 AUTO-REGENERATE SALES SCRIPT IF WEBSITE URL CHANGED
        website_url = validated_data.get('website_url')
        custom_script = validated_data.get('sales_script_text', '').strip()
        
        if website_url and website_url != instance.website_url:
            logger.info(f"🌐 Website URL changed, regenerating sales script from: {website_url}")
            try:
                # Scrape website
                website_data = scrape_website(website_url)
                ...
                if website_data.get('success'):
                    ...
                    # Generate sales script from website
                    agent_name = validated_data.get('name', instance.name)
                    agent_tone = validated_data.get('voice_tone', instance.voice_tone)
...
{website_script}"""
                        ...
                        # Only website script
                        validated_data['sales_script_text'] = website_script
                        logger.info(f"✅ Website script regenerated: {len(website_script)} characters")
                    
                    # Update website data in business_info
                    business_info = validated_data.get('business_info', instance.business_info or {})
                    business_info['website_data'] = {
                        'company_name': website_data.get('company_name'),
                        'description': website_data.get('description'),
                        'products_services': website_data.get('products_services', [])[:5],
