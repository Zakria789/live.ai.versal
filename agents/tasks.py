...
def process_scheduled_auto_calls():
    ...
    for campaign in active_campaigns:
        try:
            ...
            # Start calls with auto-scheduling
            for contact in pending_contacts:
                try:
                    # Initiate the actual call (includes auto-scheduling)
                    auto_call_view = AutoCallCampaignAPIView()
                    call_result = auto_call_view._initiate_call(contact)
                    
                    if call_result.get('success'):
                        total_calls_started += 1
                        logger.info(f"✅ Auto call started for {contact.customer_profile.phone_number}")
                        
                        if call_result.get('auto_scheduled'):
                            logger.info(f"🔄 Next calls auto-scheduled for campaign {campaign.name}")
                    else:
...
@shared_task
def simple_follow_up_call(phone_number, agent_id, previous_outcome):
    """
    Simple follow-up call task for HumeAI + Twilio integration
    Simplified version when full system is not available
    """
    logger.info(f"📞 Simple follow-up call to {phone_number} (previous: {previous_outcome})")
    
    try:
        # Import HumeAI Twilio functions
        ...
        if result['success']:
            logger.info(f"✅ Follow-up call initiated: {result['call_sid']}")
            return {
                'success': True,
            ...
            logger.error(f"❌ Failed to initiate follow-up call: {result.get('error')}")
            return {
                'success': False,
        ...
        logger.error(f"❌ Simple follow-up call failed: {str(e)}")
        return {
            'success': False,
            'error': str(e),
            'phone_number': phone_number
