...
def delete_agent(request, agent_id):
    """Delete agent from database and HumeAI"""
    try:
        agent = Agent.objects.get(id=agent_id, owner=request.user)
        ...
        # 🔥 DELETE FROM HUMEAI FIRST (if outbound agent with config)
        hume_deleted = False
        if agent_type == 'outbound' and hume_config_id:
            try:
                from HumeAiTwilio.hume_agent_service import hume_agent_service
                import logging
                
                logger = logging.getLogger(__name__)
                logger.info(f"🗑️ Deleting HumeAI config: {hume_config_id} for agent: {agent_name}")
                
                # Delete from HumeAI
                result = hume_agent_service.delete_agent(hume_config_id)
                
                if result:
                    logger.info(f"✅ HumeAI config deleted successfully: {hume_config_id}")
                    hume_deleted = True
                else:
                    logger.warning(f"⚠️ Failed to delete HumeAI config: {hume_config_id}")
                    
            except Exception as hume_error:
                logger = logging.getLogger(__name__)
                logger.error(f"❌ HumeAI deletion error: {hume_error}")
                # Continue with local deletion even if HumeAI deletion fails
        ...
        return Response({
            ...
            'hume_deleted': hume_deleted,
            'details': {
                'agent_name': agent_name,
                'agent_type': agent_type,
                'local_deleted': True,
                'hume_config_deleted': hume_deleted if hume_config_id else None
            }
        }, status=status.HTTP_200_OK)
