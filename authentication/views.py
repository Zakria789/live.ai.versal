...
def send_verification_email(user):
    ...
    try:
        ...
        subject = 'Verify Your Email - Sales AICE'
        message = f'''
Hello {user.first_name or user.email},

Thank you for signing up with Sales AICE!
...
def register_view(request):
    ...
    if serializer.is_valid():
        ...
        if email_sent:
            ...
            response_data['email_verification'] = 'Account created but verification email failed. You can still login.'
...
@swagger_auto_schema(
    ...
    request_body=openapi.Schema(
        type=openapi.TYPE_OBJECT,
        properties={
            'uidb64': openapi.Schema(type=openapi.TYPE_STRING, description='User ID (base64 encoded)'),
            'token': openapi.Schema(type=openapi.TYPE_STRING, description='Verification token')
        },
        required=['uidb64', 'token']
    ),
    responses={
        200: "Email verified successfully",
        400: "Invalid or expired token"
    },
    operation_description="Verify user email address with token from email",
