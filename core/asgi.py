...
try:
    ...
    print("[OK] Loaded calls WebSocket routing")
except ImportError as e:
    print(f"[WARNING] Could not import calls.routing - {e}")
    calls_websocket_patterns = []

try:
    from HumeAiTwilio.routing import websocket_urlpatterns as hume_websocket_patterns
    print("[OK] Loaded HumeAiTwilio WebSocket routing")
except ImportError as e:
    print(f"[WARNING] Could not import HumeAiTwilio.routing - {e}")
...
if all_websocket_patterns:
    print(f"[ROUTES] Registered {len(all_websocket_patterns)} WebSocket route(s):")
    for pattern in all_websocket_patterns:
        print(f"   - {pattern.pattern}")
else:
    print("[WARNING] No WebSocket patterns registered!")
...
print("[OK] ASGI application configured successfully")
print("="*80)
