...
class ComprehensiveDashboardAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            # Filter by: direct user OR agent's created_by user
            current_cycle_calls = TwilioCall.objects.filter(
                started_at__date__gte=billing_start,
                started_at__date__lte=billing_end
            ).filter(
                Q(user=user) | Q(agent__created_by=user)  # Direct user OR agent created by user
            )
            ...
            # Calculate actual minutes used from call data
            ...
            if user_subscription:
                ...
                # Use calculated minutes instead of subscription field
                plan_minutes_used = actual_minutes_used
    ...
    def _get_weekly_call_trends_user(self, user):
        ...
        for i in range(7):
            ...
            # Filter by: direct user OR agent's created_by user
            day_calls = TwilioCall.objects.filter(
                started_at__date=current_date
            ).filter(
                Q(user=user) | Q(agent__created_by=user)
            )
            
            inbound = day_calls.filter(direction='inbound').count()
            outbound = day_calls.filter(direction__in=['outbound', 'outbound_api']).count()
    ...
    def _get_call_type_distribution_user(self, user):
        ...
        # Filter by: direct user OR agent's created_by user
        month_calls = TwilioCall.objects.filter(
        ...
        inbound_count = month_calls.filter(direction='inbound').count()
        outbound_count = month_calls.filter(direction__in=['outbound', 'outbound_api']).count()
